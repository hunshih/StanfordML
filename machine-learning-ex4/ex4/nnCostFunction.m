...
a3 = sigmoid(z3);
for k = 1:num_labels
	sig1 = (-1 * vec_y(:, k))' * log(a3(:, k));
	one_vec = ones(m, 1);
	one_mat = ones(m, 10);
	sig0 = (one_vec - vec_y(:, k))' * log(one_vec - a3(:,k));
	J = J + (sig1 - sig0);
endfor
subTheta1 = Theta1(:, 2:end);
subTheta2 = Theta2(:, 2:end);
J = J/m + (lambda/(2*m)) * (sum(sum(subTheta1 .* subTheta1)) + sum(sum(subTheta2 .* subTheta2)));
